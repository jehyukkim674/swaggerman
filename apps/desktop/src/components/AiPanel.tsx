...
export function AiPanel({ provider, buildContext, onApplySuggestion, paramNames = [], onMentions, specUrl, pendingPrompt, onPendingConsumed, onCopyCurl, onSaveVars, claudePath }: Props) {
  ...
  function handleChat(question: string) {
    ...
    handleRef.current = provider.chat(
      { reqId: reqCounter++, prompt, system: CHAT_SYSTEM, model, sessionId: sessionRef.current, claudePath: claudePath || undefined },
      onEvent,
    );
  }
  ...
  useEffect(() => {
    ...
    provider.detect().then((d) => {
      if (alive) setUnavailable(!d.claude && !claudePath);
    }).catch(() => { if (alive) setUnavailable(!claudePath); });
    return () => { alive = false; };
  }, [provider, claudePath]);
