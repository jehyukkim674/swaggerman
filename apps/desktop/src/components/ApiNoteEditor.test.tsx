...
describe("ApiNoteEditor", () => {
  ...
  it("key가 바뀌면(다른 API로 전환) 펼침 상태가 리셋되어 빈 노트는 접힌다", () => {
    const { rerender } = render(
      <ApiNoteEditor key="api-a" note={{ text: "A 메모", status: "review", updatedAt: 1 }} onChange={vi.fn()} />,
    );
    expect(screen.getByPlaceholderText(/메모/)).toBeTruthy(); // 펼침

    rerender(
      <ApiNoteEditor key="api-b" note={emptyNote()} onChange={vi.fn()} />,
    );
    expect(screen.queryByPlaceholderText(/메모/)).toBeNull(); // 접힘
  });
});
