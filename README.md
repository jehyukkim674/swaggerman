...
- 요청 편집·전송(임의 호스트, CORS 우회) + 응답 표시·JSON 뷰어·스키마 검증·**표(Table) 보기**
- 히스토리(검색·필터·비교) / 컬렉션(Postman 호환, 4개 포맷 내보내기) / 러너
- 환경·변수 치환 `{{}}` / 요청 체이닝 / 어서션 / 인증(Bearer·Basic·API Key·OAuth2) · **인증 플로우 토큰 갱신(🔑)**
- **Mock 서버** · **프록시 녹화**(브라우저 CDP 캡처) · **플로우 빌더** · **API 성능 추이** · **시간여행** · **가이드 문서 생성**
- **AI 어시스턴트** — 설명/진단/폼 채우기/채팅, "✦ API 설명 · ✦ 고쳐줘 · ✦ /찾아(자연어 검색)"(로컬 `claude` CLI)
- **전체 백업/복원** · 자동 업데이트 · 멀티윈도우 · 커맨드 팔레트(⌘K) · 전역 단축키 · 다크/라이트 테마
