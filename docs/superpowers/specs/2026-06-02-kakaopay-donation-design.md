...
## 수정 (v0.3.24): QR 코드 방식으로 변경

**문제**: `qr.kakaopay.com` 링크는 **모바일 전용**(PC 브라우저에서 404). PC 데스크톱 앱에서 브라우저로 열면 동작하지 않음.

**해결**: 링크 열기 대신 **QR 코드 표시** — 휴대폰 카메라/카카오톡으로 스캔하면 폰에서 카카오페이가 열린다.

- **QR 생성**: python qrcode로 URL을 29×29 모듈 SVG path로 변환, 정적 데이터로 번들 (런타임 의존성 없음). opencv로 디코드 검증 완료.
- **`DonationQR.tsx`**: 인라인 SVG QR 컴포넌트 (흰 배경 + 검은 모듈, 테마 무관)
- **`DonationModal.tsx`**: QR + "휴대폰으로 스캔" 안내 모달
- **상단 배너**: [☕ 커피 사주기] 클릭 → 브라우저 대신 DonationModal 열기 (배너는 dismiss)
- **설정 모달 정보 섹션**: 버튼 대신 QR을 바로 인라인 표시 (모달 중첩 회피 — ESC 핸들러 충돌 방지)
- **README/Pages**: 뱃지(모바일용) 유지 + QR 이미지(PC용) 추가, "모바일 전용 링크" 안내
- `openUrl` 사용 제거 (더 이상 불필요)
